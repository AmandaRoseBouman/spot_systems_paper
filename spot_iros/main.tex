...
For detecting the aforementioned risk sources in unknown environments, we build a local terrain map of the robot's surrounding, via data collected from NSP. % for modeling the terrain up to 10m away from the robot.
...
A traversability map $g$, which was obtained during a real mission, is shown in Fig. \ref{global_costmap}. % 4
...
% \ph{Intractability} However, computing an optimal sequence of actions over the map $g$ is computationally infeasible. To obtain a tractable solution, we introduce an approximation of the environment with a information-rich bidirectional graph structure which reduces the policy search space.

% \ph{Approach} Computing the optimal sequence of actions over a dense map, such as $g$, is computationally infeasible since .To obtain a tractable solution, we introduce an approximation of the environment with a information-rich bidirectional graph structure which reduces the policy search space.
...
% For the frontier $v_f$ with planning depths $K$, we can write the belief of the frontier to be $b_{f,k} = p(M_k, x_k = x_f)$. 
...
% \ph{Action Cost} The cost $C(E; \pi^g(v_i))$ associated with an action $\pi^g(v_i)$ is defined as:
% \begin{align}
%     C(E; \pi^g(v_i)) = \alpha_R \mathcal{R}(v_i,\pi^g(v_i)) + L
% \end{align}
...
The Au-Spots successfully detected and localized a total of 16 artifacts, capturing 16 points and thus propelling our team CoSTAR to a 1st-place victory. %25 artifacts across all four runs with the best scores from each course amounting to 16 points which propelled our team CoSTAR to a 1st-place victory. The Au-Spots explored a combined distance of 4km, including successfully climbing multiple flights of stairs, in all live-missions.
...
% mapping
Fig. \ref{fig:exploration_planner_topview} depicts the area explored by the robots during a Beta Course run. This particular collaboration of robots mapped a total of \rev{1.3km}.%HOW MANY 
One of the most challenging elements to map in the course were the stairs due to Au-Spot's
%Stairs present in the course are one of the most challenging course elements from the mapping perspective due  to  the  
