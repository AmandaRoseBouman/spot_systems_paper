...
Joel Burdick$^{2}$, and Ali-akbar Agha-Mohammadi$^{1}$% <-this % stops a space
...
Autonomous robot mapping and traversal of extreme environments under time constraints has a wide variety of real-world applications, including search and rescue after natural disasters \cite{Nagatani2013}, exploration of extreme planetary terrains \cite{ali-agu,Taka2020JFR,Husain2013}, and inspection of urban underground environments \cite{Kolvenbach2020}. As a concrete mission, we focus on the DARPA Subterranean (SubT) Challenge \cite{subt_webpage}: a robotic competition %\rev{which tasks competitors with autonomously exploring, mapping, and searching extreme underground environments.} 
that targets missions to explore, map, and search extreme underground environments.

% This paper discusses several aspects of Team CoSTAR's solution, focused on endowing legged platforms with high-levels of autonomy in order to accomplish objectives of complex mission such as the SubT mission in extreme environments. 

% We verified the proposed solutions in the 2020 DARPA Subterranean (SubT) Challenge, Urban Circuit \cite{subt_webpage}. Our SubT mission objective was to guide a team of robots to rapidly navigate and map an extreme underground urban environment with the goal of locating certain artifacts of interest. 

%hardware-integrated framework which endows a legged platform with high-level autonomy in order to \st{accomplish} \rev{execute} complex missions. We verified the proposed solutions in the 2020 DARPA Subterranean (SubT) Challenge, Urban Circuit \cite{subt_webpage}. Our SubT mission objective was to guide a team of robots to rapidly navigate and map an extreme underground urban environment with the goal of locating certain artifacts of interest. 

%Elements of these extreme environments that necessitate novel mobility capabilities development. These requirements include:
...
\begin{figure}[t!]
  ...
  Live-mission image of Autonomous Spot robot climbing down four flights of stairs in the Urban Circuit of the DARPA Subterranean Challenge. This platform is one of the elements in team CoSTAR's solution that won the Urban Circuit of this competition. (Image credit: DARPA).} %You could also say something like "Au-Spot, in an autonomous, collaborative effort with our other robots propelled our team (CoSTAR) to a 1st-place finish (Image credit: DARPA).
...
\begin{figure*}[h!]
  ...
  \caption{Overview of the NeBula system architecture enabling high-level autonomy on Spot. Red paths denote NeBula's belief-aware planning where the planner aims to minimize mission risk by generating information-seeking paths that increase the accuracy of the shared world belief. %NeBula system has 4 key blocks: Belief Manager, Perception, Planning, and Communication}.
...
%\ph{Legged Mobility}
Legged robots offer unique mobility capabilities which make them highly suitable for traversing challenging environments that would prove difficult for wheeled robots, as they have the ability to meet locomotion, size, payload, and endurance requirements to operate in extreme environments. %of the \rev{DARPA}missions.
...
%are examples of %such \rev{prominent and novel legged} %powerful systems. 
...
% \st{The researchers then successfully demonstrated autonomous point-to-point navigation in an industrial complex.}
...
%%%Tom: This is a nice way of saying they have an over-fitted solution for tunnels.  
...
We briefly discuss the NeBula (Networked Belief-aware Perceptual Autonomy) architecture and explain some of the key elements of integrating comprehensive autonomy %on 
with the Spot robot.
...
\begin{enumerate}
    ...
    % We refer to the combined NeBula payload and Spot robot in this paper as Autonomous-Spot or Au-Spot.%, to facilitate extreme exploration and high-level sensing, communication,\rev{and computation.} \st{and computational \rev{necessities}.}
    ...
    %\item Demonstrating real-world complex Addressing challenges in operating multi-robot simultaneously, exploring complex environments, and intermittent communication by deploying Nebula's mission planning modules
    %\item Discussing what we learned during design, testing, and operation of Nebula's powered Spot
\end{enumerate}
The performance of these technologies was successfully field-tested at the Urban Circuit of the DARPA SubT Challenge (and practice runs leading to the competition), as part of team CoSTAR's solution.%, leading to first place.%  propelling our team CoSTAR to a 1st-place victory.
...
%To deal with uncertainty in unknown environments, NeBula employs a probabilistic approach. It takes the uncertainty into account to probabilistically fuse various sensing modalities, creates a probabilistic representation of the robot's knowledge of the environment, computes risk, and "proactively" plans to minimize the mission risk. 
...
% This is in addition to planner's typical objectives of minimiz such as project the belief from the feedback of planning modules.
% We will discuss these modules and highlight their uncertainty- and perception-aware aspects in Sections \ref{sec:local_planning} and \ref{sec:coverage}. 
...
%\st{Spot can negotiate challenging non-planar terrain on a variety of surface types, including stair climbing.} \rev{In particular, Spot is capable of ascending and descending stairs.}
...
% \st{Considering Spot’s overall traversability capabilities, mechanical constraints, and integration complexities, NASA JPL's NeBula Sensor Package (NSP) provides %a vehicle-agnostic and 
% an elevated additional perception system (Fig. \ref{fig:spot_full_nebula_form}) to enable higher levels of autonomy.}
To enable higher levels of autonomy required in the SubT challenge, we augment Spot's in-built sensing package with NeBula Sensor Package (NSP). %a vehicle-agnostic and 
% an \st{elevated} \rev{additional} perception system (Fig. \ref{fig:spot_full_nebula_form}) to enable higher levels of autonomy.
...
% \st{Spot is a fast-moving quadruped with the ability to traverse rough terrain and With NeBula autonomously driving this capability to its limit, the NSP can experience significant forces, moments, and vibrations.}
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=0.4\textwidth]{spot_iros/graphics/coverage_matteo.png}  % .pdf one is with scale
...
% \begin{figure}[t!]
%   \centering
%   \includegraphics[width=0.35\textwidth]{spot_iros/graphics/coverage_matteo.png}  % .pdf one is with scale
%   %\caption{Area explored by a fleet of two Spots and two wheeled robots.  Colored pointclouds indicate explored areas, with colors indicating z height, spanning 3 floors. (Image credit: DARPA).}
%   \caption{
% %   Area explored in one of the SubT challenge live missions by a team of two UGV's and one Au-Spot. Colors indicate robot-specific coverage (Red: UGV 1, Blue: UGV 2, Yellow: Au-Spot 1). 
  
% %   This coverage map .... this is one level of 3 or 4...
% %   note that... there are lots of traversability stressing elements, narroe passage sand rooms that robot need to peek in and saerch and explore...
% %   there are lots of re-explore and search ... path is loopy...
% %   path is zig-zag... - 
  
%   Top view of area covered in urban environment by robot fleet consisting of one Au-Spot [yellow] and two UGVs [red/blue] during one run of the SubT Challenge, Urban Circuit. The map contains three layers, each representing a different floor of the environment, connected by multi-flight staircases. Note the complex topology (narrow passages, varying-sized interconnected rooms, and the outer circular geometry) requiring an irregular exploratory behavior that is characterized by many subrountes for side room inspection. Representation of a known-explored and known-unexplored areas. 
  
%   The global coverage planner must maintain a history of the explored areas 
  
% %   Colors indicate robot-specific coverage (Red: UGV 1, Blue: UGV 2, Yellow: Au-Spot 1). 
...
\ph{Coverage Planner}
Au-Spot's coverage planner successfully guided a fleet of two Au-Spots and two wheeled UGV robots to collaboratively explore and map a large unstructured environment within the one hour time limit in the SubT Challenge. Fig. \ref{fig:exploration_planner_topview} depicts the area explored by the robots during one of the four live-mission runs. In this single (one hour) run, the fleet of robots mapped a total volume of approximately 25,000 ${m}^{3}$. %distance of 1.3km. 
...
%.  The human operator sees a staircase in Au-Spot's sensory data, and commands Au-Spot to move down the stairs, with NeBula maintaining situational awareness.
%A staircase is detected by the human operator from the sensory data, and Au-Spot is sent down the stairs.
%After collecting sensor data, a staircase is detected by the human operator, Au-Spot is then sent down the staircase.
%After descending four flights of stairs in semi-autonomous mode, a communication node is deployed.
%From this point onward, Au-Spot operates without any operator intervention.
%, and thoroughly explores the lower levels of the industrial power plant.
% Au-Spot's coverage and traversability planners enable it to thoroughly explore the environment, which includes challenging features such as hallways with narrow openings, large open rooms, raised platforms, and cluttered rubble.
% NeBula detects and localizes artifacts to within a 5 m accuracy over a 1 km distance traveled, which allowed TEAM-CoSTAR to achieve a winning score.
%enters a larger room, which is explored in detail.
%Finally, Au-Spot explores a small cluttered room. %Au-Spot achieved a final score of X over the Alpha Courses, the highest of any team's robots.
...
% which takes coupled uncertainties into account in various autonomy components, including odometry, traversability, and coverage planning. %This system-focused paper reports our work on endowing high-level autonomy and perception capabilities on the Boston Dynamics Spot robot that won the Urban Circuit of the DARPA Subterranean (SubT) Challenge.
% Specific aspects of NeBula on legged robot autonomy including %including in enabling legged autonomy for complex missions, inl.  
...
% We believe this work takes an important step in advancing the state-of-the-practice by demonstrating the capabilities of legged robots for accomplishing complex, real-world, live-missions in extreme environments.

% This work aims at advancing the state-of-the-practice in capabilities of legged robots for accomplishing complex, real-world, live-missions in extreme environments.

% This system-focused paper reports our efforts to endow theBoston Dynamics Spot robot with high-levels of autonomy,hardware  and  perception  capabilities  that  won  the  UrbanCircuit of the DARPA Subterranean Challenge. In this paper,we  discussed  our  NeBula  autonomy  architecture  which  weaugmented to Boston Dynamics’ Spot and described specificaspects in enabling legged autonomy for complex missions.We believe this work is an important step in demonstratingthe capabilities of legged robots for accomplishing complex,real-world live-missions in extreme environments.
